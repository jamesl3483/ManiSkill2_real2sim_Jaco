...
class OpenDoorInSceneEnv(CustomSceneEnv):
    drawer_ids: List[str]
    ...
    def __init__(
        ...
        station_name: float = "simple_cabinet",
        # station_name: float = "cabinet/cabinet_0004",
    ...
    def _setup_prepackaged_env_init_config(self):
        ...
        ret["shader_dir"] = "rt"
        self.station_name = "simple_cabinet"
    ...
    def _load_articulations(self):
        ...
        print(f"Loading cabinet from {filename}")
        ...
        self.drawer_obj = get_entity_by_name(
            self.art_obj.get_links(), f"{self.drawer_id}_door"
        )
        self.joint_names = [j.name for j in self.art_obj.get_active_joints()]
        self.joint_idx = self.joint_names.index(f"{self.drawer_id}_door_joint")
    ...
    def reset(self, seed=None, options=None):
        ...
        info.update(
            {
                "door_pose_wrt_robot_base": self.agent.robot.pose.inv()
    ...
    def get_language_instruction(self, **kwargs):
        return f"open {self.drawer_id} door"


@register_env("OpenDoorCustomInScene-v0", max_episode_steps=113)
class OpenDoorCustomInSceneEnv(OpenDoorInSceneEnv, CustomOtherObjectsInSceneEnv):
    drawer_ids = ["door_hinge"]


class CloseDoorInSceneEnv(OpenDoorInSceneEnv):
    ...
    def get_language_instruction(self):
        return f"close {self.drawer_id} door"


@register_env("CloseDoorCustomInScene-v0", max_episode_steps=113)
class CloseDoorCustomInSceneEnv(CloseDoorInSceneEnv, CustomOtherObjectsInSceneEnv):
    drawer_ids = ["cabinet"]
