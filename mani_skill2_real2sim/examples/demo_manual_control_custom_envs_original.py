...
# JACO
python mani_skill2_real2sim/examples/demo_manual_control_custom_envs.py -e GetWaterCustomInScene-v0 -c arm_pd_ee_delta_pose_align_interpolate_by_planner_gripper_pd_joint_pos  --enable-sapien-viewer -o rgbd robot jaco sim_freq @501 control_freq @3 scene_name frl_apartment_stage_simple rgb_overlay_mode debug rgb_overlay_path data/real_inpainting/open_drawer_b0.png rgb_overlay_cameras 3rd_view_camera model_ids a_cups
...
def main():
    ...
    if (not hasattr(env, "prepackaged_config")) or (not env.prepackaged_config):
        ...
        if names_in_env_id_fxn(["GraspSingle"]):
        ...
        elif names_in_env_id_fxn(["Drawer", "Water"]):
            init_rot_quat = [0, 0, 0, 1]
            if not names_in_env_id_fxn(["PlaceInClosedDrawer", "Water"]):
                init_xy = [0.851, 0.035]
            else:
                init_xy = [0.652, 0.009]
            env_reset_options = {
            ...
            # This one is not being used now
            elif env.robot_uid == "jaco":
                env_reset_options = {
                    "obj_init_options": {},
                    "robot_init_options": {
                        "init_xy": [0.147, 0.028],
